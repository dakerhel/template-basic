...
class AppFontSettings {
  const AppFontSettings({this.scaleIndex = 3, this.family = FontFamily.system});
...
final fontProvider = NotifierProvider<FontController, AppFontSettings>(
  FontController.new,
);
...
final class FontController extends Notifier<AppFontSettings> {
  ...
  Future<void> setScaleIndex(int index) async {
    state = AppFontSettings(
      scaleIndex: index.clamp(1, 5),
      family: state.family,
    );
    final prefs = await SharedPreferences.getInstance();
